...
class BookPurchaseConsumerTest {
  ...
  void shouldReceiveSuccessMessageFromKafkaSuccessfully() throws JsonProcessingException {
    ...
    kafkaTemplate.send(
        "some-test-topic",
        objectMapper.writeValueAsString(new BuyBookMessageResponse(1L, true, "Success message")));
  ...
  @Test
  void shouldReceiveFailMessageFromKafkaSuccessfully() throws JsonProcessingException {

    kafkaTemplate.send(
        "some-test-topic",
        objectMapper.writeValueAsString(new BuyBookMessageResponse(1L, false, "Fail message")));
