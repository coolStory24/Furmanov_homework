...
class BookRatingProducerTest {
  ...
  void shouldSendMessageToKafkaSuccessfully() {
    assertDoesNotThrow(() -> bookRatingProducer.sendBookRatingCalculationRequest(1L));
    ...
    records
        ...
        .forEachRemaining(
            record -> {
              BookServiceMessageRequest message;
              try {
                message = objectMapper.readValue(record.value(), BookServiceMessageRequest.class);
              } catch (JsonProcessingException e) {
                throw new RuntimeException(e);
              }
              assertEquals(new BookServiceMessageRequest(1L), message);
            });
  }
